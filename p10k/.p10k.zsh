# Generated by Powerlevel10k configuration wizard on 2021-12-01 at 04:47 CST.
...
# 2 lines, disconnected, sparse, concise, transient_prompt, instant_prompt=off.
...
() {
  ...
  typeset -g POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(
    ...
    sukka_npm_type          # defined in .zshrc
  ...
  typeset -g POWERLEVEL9K_PROMPT_CHAR_ERROR_{VIINS,VICMD,VIVIS,VIOWR}_FOREGROUND=005
  ...
  function my_git_formatter() {
    ...
    if [[ -n $VCS_STATUS_LOCAL_BRANCH ]]; then
      ...
      res+="%004F${(g::)POWERLEVEL9K_VCS_BRANCH_ICON}${branch//\%/%%}"
    fi
    ...
    (( VCS_STATUS_NUM_UNSTAGED   )) && res+="${modified}x${VCS_STATUS_NUM_UNSTAGED}"
    ...
    (( !VCS_STATUS_COMMITS_BEHIND && !VCS_STATUS_COMMITS_AHEAD && !VCS_STATUS_PUSH_COMMITS_BEHIND && !VCS_STATUS_PUSH_COMMITS_AHEAD && !VCS_STATUS_NUM_CONFLICTED && !VCS_STATUS_NUM_STAGED && !VCS_STATUS_NUM_UNSTAGED && !VCS_STATUS_NUM_UNTRACKED && !VCS_STATUS_HAS_UNSTAGED )) && res+="${clean}o"

    # Display the current Git commit if there is no branch and no tag.
    # Tip: To always display the current Git commit, delete the next line.
    # [[ -z $VCS_STATUS_LOCAL_BRANCH && -z $VCS_STATUS_TAG ]] &&  # <-- this line
    res+="${meta}) %020F${VCS_STATUS_COMMIT[1,8]}"

    # Show tracking branch name if it differs from local branch.
    if [[ -n ${VCS_STATUS_REMOTE_BRANCH:#$VCS_STATUS_LOCAL_BRANCH} ]]; then
      res+="${meta}:%020F${(V)VCS_STATUS_REMOTE_BRANCH//\%/%%}"
    fi
  ...
  typeset -g POWERLEVEL9K_CONTEXT_FOREGROUND=046
  ...
  # Custom icon.
  typeset -g POWERLEVEL9K_PROXY_VISUAL_IDENTIFIER_EXPANSION='proxy'
